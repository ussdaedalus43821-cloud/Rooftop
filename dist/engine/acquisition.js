...
const TIER_QUOTA_MULTIPLIER = {
    bronze: 0.56,
    silver: 1,
    gold: 1.44,
    platinum: 2,
};
export function tierMonthlyAllocationCap(baseQuota, tier) {
    return Math.max(1, Math.round(baseQuota * TIER_QUOTA_MULTIPLIER[tier]));
}
function generateVin(rng) {
    const chars = "ABCDEFGHJKLMNPRSTUVWXYZ0123456789";
    let vin = "";
    for (let i = 0; i < 17; i++)
        vin += chars[rng.int(0, chars.length - 1)];
    return vin;
}
export function allocationCatalog(d) {
...
export function allocationRemainingThisMonth(d) {
    return Math.max(0, d.manufacturer.allocationCapMonthly - d.manufacturer.allocationOrderedThisMonth);
}
export function makeVehicle(model, condition, source, day, rng, odometer) {
    return {
        id: nextId("veh"),
        model,
        condition,
...
export function orderAllocationUnit(d, model, day, rng) {
    if (allocationRemainingThisMonth(d) <= 0)
        return null;
...
export function generateAuctionLots(rng, day, count = 6) {
    const lots = [];
...
export function bidOnAuctionLot(d, lot, bid, day, rng) {
...
export function createTradeInVehicle(d, model, odometer, appraisedValue, day, rng) {
