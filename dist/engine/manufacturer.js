...
export function monthlyManufacturerCycle(d, day) {
    if (d.manufacturer.terminated)
        return;
...
function setTier(d, tier, day) {
    if (d.manufacturer.tier === tier)
        return;
...
function terminateFranchise(d, day) {
    d.manufacturer.terminated = true;
    d.isUsedOnly = true;
    d.manufacturer.allocationCapMonthly = 0;
}
const FACILITY_INVESTMENT_COST = 12000;
export function investInFacilityStandards(d) {
    if (d.ledger.cash < FACILITY_INVESTMENT_COST || d.manufacturer.facilityStandards >= 100)
        return false;
...
export function facilityInvestmentCost() {
