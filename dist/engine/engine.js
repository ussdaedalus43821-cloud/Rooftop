...
import { accruePayroll, payAccruedPayroll, payFloorPlanInterest, postCashExpense, } from "./financials.js";
import { OVERHEAD_MONTHLY } from "../constants.js";
const MAX_TOASTS = 30;
const STALE_NEGOTIATION_DAYS = 3;
export function pushToast(state, text, kind = "info") {
...
function tickDealershipDay(state, d, rng) {
    ...
    d.deals = d.deals.filter((deal) => !((deal.stage === "closed" || deal.stage === "lost") && state.day - deal.createdDay > 14));
    if (d.failure) {
        pushToast(state, `${d.name}: the floor-plan lender has swept unpaid inventory. The line is pulled.`, "bad");
    }
}
function finalizeMonth(state, d) {
    ...
    if (d.monthlyHistory.length > 36)
        d.monthlyHistory.shift();
...
export function advanceOneDay(state, rng) {
    state.day += 1;
    ...
    if (isNewMonth(state.day)) {
        for (const id of Object.keys(state.dealerships)) {
            ...
            pushToast(state, `${d.name} — ${justFinalized.monthLabel}: net ${justFinalized.netIncome >= 0 ? "profit" : "loss"} of $${Math.abs(Math.round(justFinalized.netIncome)).toLocaleString()}.`, justFinalized.netIncome >= 0 ? "good" : "warn");
        }
