...
export function isNewGameSetupActive() {
    return newGameSetupActive;
}
export function mountApp(root, state, rng, markDirty, forceSetup = false) {
    rootEl = root;
    ctx = {
        state,
        rng,
...
function roleLabel(state) {
    ...
    if (state.career.role === "partial_owner")
        return `Partial Owner (${Math.round(state.career.equityPct * 100)}%)`;
    return "Owner-Operator";
}
function renderMilestoneModal() {
    if (!ctx || !ctx.state.career.milestoneOfferPending)
        return "";
...
function onClick(e) {
    ...
    if (action === "global:setSpeed") {
        ctx.state.speed = Number(target.getAttribute("data-speed"));
        render();
        return;
    }
    ...
    if (action === "global:newGame") {
        if (!confirm("Start a brand-new store? This discards the current game."))
            return;
        newGameSetupActive = true;
        render();
        return;
    ...
    if (action === "newgame:pick") {
        const key = target.getAttribute("data-franchise");
        clearSave();
